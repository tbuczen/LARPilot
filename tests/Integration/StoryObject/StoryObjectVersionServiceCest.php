...
class StoryObjectVersionServiceCest extends Unit
{
    public function versionHistoryDiffCalculatedCorrectly(FunctionalTester $I): void
    {
        $I->wantTo('verify that version history diffs are calculated correctly');
        ...
        $I->assertCount(2, $history);
        $I->assertSame('Hero Updated', $history[0]['entry']->getData()['title']);
        $I->assertSame('Hero', $history[1]['entry']->getData()['title']);
        $I->assertSame(['old' => 'Hero', 'new' => 'Hero Updated'], $history[0]['diff']['title']);
    }
}
